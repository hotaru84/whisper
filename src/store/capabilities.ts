 ...
 *
 * Deliberately only three values, with failures (`errorMessage`) kept on
 * their own axis. There used to be a second axis here, `ProcessingPhase`,
 * for the post-stop analysis pipeline -- removed once recording and analysis
 * became independent (see `src/store/analysisQueue.ts`): a take's own
 * lifecycle no longer has anything to do with whether *some* recording,
 * possibly a different one, has analysis running in the background.
 */
 ...
 *
 * Deliberately excludes anything about a *specific* recording's analysis
 * (queued/running/cancellable) -- that is per-recording state now
 * (`src/store/analysisQueue.ts`'s `AnalysisJob`), not a global capability.
 */
export interface Capabilities {
  ...
  playback: boolean;
  /** Always available while stopped -- enqueues a background job rather than
   * running synchronously, so it never needs to wait for any other
   * recording's analysis. Per-recording de-duplication (don't enqueue a
   * second job for a recording that already has one) is
   * `analysisQueue.hasActiveJob`'s job, not this. */
  reanalyze: boolean;
...
export interface CapabilityInputs {
   ...
   * about the other axes don't have to plumb it through; only the
...
export function selectCapabilities(s: CapabilityInputs): Capabilities {
  ...
  return {
    ...
    // `recordingPhase` itself flips, would otherwise still read as `stopped`.
    //
    // Deliberately *not* gated on any recording's analysis state: a previous
    // take's refine/diarize/audio-tag pass (or a history re-analysis) running
    // in the background must never block starting a new one -- that decoupling
    // is the entire point of `src/store/analysisQueue.ts`'s job queue. Model
    // access itself is arbitrated by `src/lib/asr/whisperQueue.ts`, not by
    // this capability.
    startRecording:
      stopped && !s.startingRecording && (s.directoryConfigured ?? true) && (s.recordOnly || s.modelStatus === "ready"),
