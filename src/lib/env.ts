 ...
 *   the sidebar is not empty on first load
 ...
 * - `export/saveTranscript.ts` -- refuses (no native save dialog); the menu
 *   items are disabled to match
 *
...
if (useMockBackend) {
  console.info(
    "[env] Tauri backend not detected -- falling back to mock ASR/capture/history/playback so the frontend can be reviewed in a plain browser. Real transcription, playback audio, app-audio capture, transcript export, and window controls will not work.",
  );
}
