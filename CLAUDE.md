...
### Process split
- **Rust backend (`src-tauri/src/`)** owns: model loading, all whisper inference, diarization, audio-event detection, WAV file writing, and WASAPI endpoint-loopback capture of whatever plays through the default output device (e.g. Teams/Zoom). Exposed to the frontend as Tauri commands, registered in `src-tauri/src/lib.rs`.
