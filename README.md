...
停止後の後処理パスで、録音全体を音楽・拍手・ノイズなどのタグ付け（audio tagging）にかける。**設定パネルでは
...
- **CER と別に、正解テキスト不要の構造指標(`src-tauri/src/cues.rs`)も同じハーネスで出る。** ゼロ長キュー数・キューの
  ...
  `asr::finalize_transcript` の結果にも同じ関数で計算した値が乗る(フロントエンドの `quality` フィールド)。
...
読み込みは本体・ハーネスともに `src-tauri/src/wav.rs` を共有するので、測った値と本番の挙動が食い違わない。
...
- **モデルは Rust バックエンドが直接ロードする。** `src-tauri/src/asr.rs` の `init_model` コマンドが
  ...
  不要になった。
- **録音・解析を問わず、文字起こしは常に同じ1本のウィンドウ処理パスを通る。** マイク入力は `MediaRecorder`
  （WebM/Opus）ではなく `AudioContext({ sampleRate: 16000 })` + AudioWorklet（`public/pcm-capture-worklet.js`、
  `src/lib/audio/pcmRecorder.ts`）で 16kHz mono の生 PCM として取り込む。録音全体をメモリに保持せず、フレームは
  `StreamingTranscriber` (`src/lib/asr/streaming.ts`) に流し込み、30秒たまるごとにウィンドウを文字起こしして
  「確定した部分だけコミット・末尾は次ウィンドウへ持ち越し（chunk-and-commit）」する。ウィンドウは互いに独立に
  デコードされる（呼び出しのたびに新しい `whisper_state`）ため、ウィンドウ境界をまたぐ文はどちら側でも文脈を
  欠いたまま推測される——録音全体を通した文脈を持つ処理は存在しない。
  - **ウィンドウ長は以前15秒だったが、現在は30秒。** Whisper は常に30秒分をエンコードし短い音声は無音で
    パディングするので、GPU コストはウィンドウ長によらず約2秒。以前は「最初の文字が出るまでの待ち時間」を
    半分にするため15秒にしていたが、文字起こし呼び出しは優先度付きキュー（`src/lib/asr/whisperQueue.ts`、
    ライブ録音のウィンドウが常に背景ジョブより優先される）を経由するようになり、サブ秒単位の応答性は
    もう要件ではなくなった。トレードオフなしに30秒へ戻せる（ウィンドウが大きいほど境界で文が切れる頻度が減る）。
  - 各ウィンドウの音声は JSON ではなく Tauri の生バイナリ IPC（`tauri::ipc::Request`）で Rust 側へ転送し、
    言語/タスク設定は HTTP ヘッダーで渡す（`transcribe_window` コマンド）。worklet は CSP `script-src 'self'`
    を満たすため `public/` から同一オリジンで配信する（`data:` へインライン化されると CSP に弾かれる）。
  - **記録専用（record-only）モードの「解析」、履歴の「再解析」も、この同じ `StreamingTranscriber` を
    WAV ファイルの PCM（`read_wav_pcm` コマンド）で駆動するだけ**（`src/lib/asr/postHocTranscriber.ts`）。
    ライブのマイク入力を WAV 読み出しに差し替えているだけで、デコード自体は完全に同じ経路を通る。
...
- **以前は録音停止後に、録音全体を1回の `full()` に渡す「全体精度パス」（`asr::transcribe_recording`）が
  もう1本走っていたが、廃止した。** whisper.cpp が内部で30秒チャンクに割り、直前のチャンクのトークン列を
  次へ引き継ぐ（context conditioning）ため理屈の上ではウィンドウ間の文脈が効くはずだったが、その効果を
  ウィンドウ処理単体との比較で実測したことは一度も無く、かつこのパス自体に固有の失敗モードがあった。
  録音・記録専用モードの後解析・履歴の再解析のすべてが同じ `StreamingTranscriber`/`transcribe_window`
  経路を通るようになった今、廃止に伴って **VAD（音声区間検出）も同時に撤去した**（VAD はこの全体精度パス
  内の `full()` 呼び出しでしか機能しておらず、独立したウィンドウ単位の処理には構造上組み込めないため）。
  - **停止後もユーザーは録音中に出た文字起こしをそのまま読める。** 後述の後処理（無音区間フラグ付け・
    ループ修復・話者分離・音響イベント検出）が失敗しても録音中の結果は保持し、`refineNotice` として
    理由だけを添える（`src/store/appStore.ts`）。既に手元にある文字起こしを、改善のために失うことが
    あってはならない。
  - **実測（Intel Iris Xe 内蔵GPU環境）: 27.7分の会議を731秒で処理、1時間換算で約26分。** 「記録専用」で
    録音した後にまとめて解析する場合や、履歴の「再解析」がこの処理時間にあたる（ライブ録音中の逐次文字
    起こしは録音と並行して進むため、停止時点でほぼ完了しており別途この時間を要しない）。
- **停止後の後処理パス（無音フラグ付け・ループ修復・話者分離・音響イベント検出）は途中でキャンセルできる。
  中断の合図は共有の `Arc<AtomicBool>` 1本。** 話者分離と音響イベント検出は進捗を一切出さないため、
  whisper が 100% に達したあと readout は固まったまま数分待たされることがある。設定を間違えたことに
  ...
  - **フラグが1本で足りる根拠は `selectCapabilities` にある。** `reanalyze` も `startRecording` も
    ...
    ライブ録音の後処理（`src/store/recordingPipeline.ts` の `refineRecording`）と記録専用/再解析
    （同ファイルの `runPostHocAnalysis`）がそれぞれ自分の先頭で `begin_analysis` を呼んで毎回クリアする
    ことで防ぐ（両者は共通の後処理テール `finalizeAndEnrich` を末尾で共有するが、`begin_analysis` は
    それぞれの入口で個別に呼ばれる）。
  ...
  - **途中結果は捨て、ライブパスの文字起こしを残す。** 半分だけ精緻化されたセグメントをライブ結果の頭に
    ...
    既存のサイドカーはそのまま残る。
- **「録音のみ」モードは通常の逐次文字起こしをまるごと省略する形で実装している。** バッテリー駆動時に、録音中ずっと
  30秒ウィンドウの逐次パスが GPU（Vulkan）を回し続けるのが消費電力の支配項になる。これを避けるため、
  ...
  コンテキストすら作られない。
  - 停止時は `refineRecording`（録音後処理）ではなく `finishRecordOnly` を呼ぶ。WAV を閉じて再生可能にし、
...
- **録音中の音声は WAV としてディスクに追記する。** 16kHz f32 は1時間で約230MB あり、全体をメモリに置けない。
  ...
  `wav::Writer` が16bit PCM で追記する。書き込みに失敗しても録音と逐次文字起こしは続行する（失うのは停止後の後処理だけ）。
...
- **停止後の後処理は無音区間のセグメントを音声側の根拠でフラグ立てする。** 逐次パスの `isNearSilent` は
  ウィンドウ（30秒）単位でしか無音を弾けないため、ほとんど無音だが一部に音声がある窓ではこの粗い網を
  すり抜ける。そこでデコード後に、ウィンドウ処理で組み立てられたチャンク列を対象に、各セグメントの
  区間（前後1秒の余裕込み）の RMS が `SILENCE_RMS` 未満ならそのセグメントを無音としてフラグを立てる
  (`asr::mark_silent_segments`)。**判定材料はテキストではなく音声**なので、「はいはいはい」のような正当な発話を
  文面の見た目で消すことがない。前後1秒の余裕は whisper のタイムスタンプが粗いことへの保険（DTW は後述の理由で
  廃止済みなので、なおさら詰められない）。
...
- **DTW（動的時間伸縮法）は導入・修正した上で、最終的に廃止した。** whisper が既定で出すセグメント境界は
  単一タイムスタンプトークンを読むだけの粗い実装で数百ms ずれることがあり、それをアテンション行列追跡による
  トークン単位の時刻に置き換えるのが DTW だった（`large-v3-turbo` 専用プリセットあり）。導入当初は
  `asr::collect_segments` がセグメント単位の `start_timestamp()`/`end_timestamp()` しか見ておらず、DTW が書く
  `t_dtw` を一切読んでいなかったため、コスト（40秒音声・Vulkan で +4〜5%）だけ払って精度への寄与がゼロという
  期間があり、これは後に `collect_segments` を `t_dtw` の最小・最大を読むよう直して解消した。
  - **最終的に廃止した理由は abort バグ。** whisper.cpp の DTW メディアンフィルタは
    `WHISPER_ASSERT(filter_width < a->ne[2])`（whisper.cpp:8772、`filter_width` はハードコードで `7`）を通るため、
    whisper がタイミングを計算するセグメントが約160ms（`n_frames >= 16`）未満だと即座に `abort()` する。
    これは Rust 側の `Result` やパニックキャッチで捕捉できない、プロセス全体の即死。DTW はモデルロード時
    （`asr::init_model`）に一度だけ設定される context 単位の設定で、個別のデコード呼び出しごとに切ることが
    できないため、`asr::redecode_voiced_gaps`・`asr::redecode_degenerate_loops` のような短い音声片を単独で
    再デコードする経路（まさに whisper.cpp の seek が不規則に飛ぶケースへの対処として存在する経路）が、
    今度は同じ不規則な seek の結果として160ms未満のセグメントを生み、この assert を踏みうる。呼び出し側で
    音声長に下限を設けても「呼び出し全体の長さ」は保証できても「whisper.cpp 内部の最終セグメント断片の長さ」
    までは保証できないため、確実な対策は DTW を切ることのみだった。テキスト自体には寄与しない（セグメント
    境界の精度のみに効く）機能なので、話者分離の話者割り当て精度が whisper 既定の粗いセグメント境界まで
    後退するのを許容してでも、クラッシュしないことを優先した。
...
- **マイクは設定パネルから選べる。** `navigator.mediaDevices.enumerateDevices()`（`src/lib/audio/devices.ts`）で
  ...
  抜き差しに応じて一覧が自動更新される。
- **相手（Teams/Zoom 等）の音声は WASAPI エンドポイントループバックで取り込む。** マイクだけでは自分の声しか
  録れず、ヘッドセット利用時は相手の声が一切記録されない。これを解決するため、既定の再生デバイスに流れる
  音声をまるごと取り込む（`src-tauri/src/appaudio.rs`）。
  - `Chromium` の `getDisplayMedia` は Windows では画面共有のタブ音声かシステム全体の音声しか取れず、
    **ネイティブアプリの音声取得はできない**ため、Win32 の WASAPI ループバックを使う。
  - **以前は特定プロセスだけを狙う `AUDIOCLIENT_ACTIVATION_TYPE_PROCESS_LOOPBACK`
    （[`wasapi`](https://github.com/HEnquist/wasapi-rs) クレートの `AudioClient::new_application_loopback_client`、
    `include_tree: true` で子プロセスも対象）を使っていたが、Teams/Zoom の実際の通話で音声を取りこぼす不具合が
    確認されたため、既定の再生デバイス全体をキャプチャする通常の WASAPI ループバック（`DeviceEnumerator::
    get_default_device(&Direction::Render)` で得たデバイスの `AudioClient` を `Direction::Capture` で初期化する
    ――Render 方向のデバイスを Capture 用に初期化すると `AUDCLNT_STREAMFLAGS_LOOPBACK` が内部的に立つ、という
    WASAPI の標準的な仕組み）に切り替えた。** これに伴い、対象アプリを選ぶピッカー（PID 列挙・アイコン抽出）は
    撤去し、設定パネルは「システム音声を含めるか」の単純な ON/OFF トグルになっている。
  - **トレードオフ: 選んだ相手アプリ以外の音（通知音、他のアプリの再生音など）も同じ再生デバイスに流れていれば
    一緒に録音される。** 会議の音声を取りこぼして文字起こしから欠落するほうが、余計な通知音が1つ混ざるより
    悪い失敗モードだと判断し、確実性を優先した。
  - **16kHz mono への変換はリサンプラを自前で書かず WASAPI 自身にやらせている。** `wasapi` の
    ...
    ループバックモードでも機能することを自プロセスでのトーン再生＋キャプチャで実測確認済み
    （48kHz ステレオ → 16kHz モノを要求してそのまま通った）。当初計画していた `rubato` は不要と判断し、
    依存に加えていない。
  - **何も再生されていない間、ループバックは何もデータを返さない。** 放置するとマイクとの時間軸がずれるため、
  ...
  - **アプリ音声の取得は失敗しても録音全体を止めない。** 開始時に失敗すればマイクのみで録音を続け、
    録音中に何らかの理由でキャプチャが失敗すれば以降マイクのみに切り替わる（`asr:app-audio-error` イベント）。
    いずれも `refineNotice` で理由を表示する。
- **音響イベント検出も話者分離と同じく、録音全体を見る停止後の別パスとして実装している。** 逐次パスの
  30秒ウィンドウでは「この10秒は音楽か」の判定に使える文脈が足りない、というのが主な理由（`src-tauri/src/events.rs`）。
...
- **無音のウィンドウはモデルに渡さない。** `StreamingTranscriber` は RMS が `SILENCE_RMS`（`src/lib/asr/diagnostics.ts`
  ...
  設定パネルの「幻覚対策（上級者向け）」から調整でき、逐次パス（`isNearSilent`）・停止後の後処理の無音フラグ付け
...
- **反復ループ対策に `entropy_thold` を既定 2.8 へ引き上げている（whisper.cpp既定 2.4）。ただし短い反復には効かない。**
  ...
  （`transcribe_window` の `X-Entropy-Thold` ヘッダ）・停止後の後処理（`asr::finalize_transcript` が組み立てる
  `DecodeSettings`）の両方に反映される。
...
- **PC のサスペンド（スリープ）は「無言で壊れる」入力として扱う。** Windows はサスペンドを webview に通知しない。
  ...
  - **逐次パスは失敗し続けても録音を悪化させない**（`streaming.ts`）。サスペンドで Vulkan デバイスがロストすると
    ...
    駄目ならそのウィンドウを捨てて通知する。**捨てても録音は無傷**で、停止後の後処理（`asr::finalize_transcript`）が
    ファイル全体を読み直す。
